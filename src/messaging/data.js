...
module.exports = function (Messaging) {
	...
	Messaging.getMessagesData = async (mids, uid, roomId, isNew) => {
			...
			msgs
				.map((msg, idx) => {
					if (msg) {
						...
						msg.ip = undefined;
						msg.isOwner = msg.fromuid === parsedUid;
					}
					return msg;
				})
				.filter(Boolean);
	
		const attachFromUsers = (messages, users, parsedUid, roomId) => {
			messages.forEach((message, index) => {
				...
				const self = message.fromuid === parsedUid;
		...
		const setNewSetForMultiple = messages =>
		...
		const setNewSetForSingle = async (messages, roomId) => {
		...
		messages = normalizeMessages(mids, messages, parsedUid);
		...
		messages = attachFromUsers(messages, users, parsedUid, roomId);
		await parseMessages(messages, uid, roomId, isNew);
	
		if (messages.length > 1) {
			messages = setNewSetForMultiple(messages);
		} else if (messages.length === 1) {
			messages = await setNewSetForSingle(messages, roomId);
		}
